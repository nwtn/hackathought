...
<?php
	...
	foreach ($disp_custom_fields as $field_id => $field_property)
	{
		...
		{
			// KH: we only want to display the tags, so don't display text field tags
			// Text Field
			echo "<div class=\"text-field report_row\" id=\"custom_field_row_" . $field_id ."\">";
			...
			switch ($field_property['field_type'])
			{
				...
				case 6:
					...
					foreach($options as $option)
					{
						...
						$html .= "<span style=\"margin-right: 15px\">";
						$html .= "<label>";
						//$html .= form::checkbox("custom_field[".$field_id.'-'.$cnt.']', $option, $set_default, ' id="custom_field['.$field_id.'-'.$cnt.']" ');
						//$html .= form::label("custom_field[".$field_id.'-'.$cnt.']'," ".$option);
